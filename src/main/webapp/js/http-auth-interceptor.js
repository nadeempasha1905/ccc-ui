...
(function () {
  ...
  .config(['$httpProvider', function($httpProvider) {
    $httpProvider.interceptors.push(['$rootScope', '$q', 'httpBuffer','$state','store','$cookies',
                                     function($rootScope, $q, httpBuffer,$state,store,$cookies) {
      return {
        responseError: function(rejection) {
          ...
          if (!config.ignoreAuthModule) {
            ...
            case 401:
            	if(rejection.data.error_description.includes("Invalid refresh token (expired)")){
            		$cookies.remove("access_token");
            		store.remove('userInfo');
            		store.remove('facilityinfo');
            		$rootScope.isLoggedIn = false;
            		$rootScope.isHFUser = false;
